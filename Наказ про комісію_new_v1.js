function onBeforeCardSave() {
  ...
  setAttrValue(
    "ApprovalNameI",
    formattingOfInitials(EdocsApi.getAttributeValue("Approval")?.text)
  );
  setAttrValue(
    "AddReviewNameI",
    formattingOfInitials(EdocsApi.getAttributeValue("AddReview")?.text)
  );
  setAttrValue(
    "TeacherName2I",
    formattingOfInitials(EdocsApi.getAttributeValue("TeacherName2")?.text)
  );
  setAttrValue(
    "StdName2I",
    formattingOfInitials(EdocsApi.getAttributeValue("StdName2")?.text)
  );
}
...
function formattingOfInitials(fullName) {
  ...
  if (fullName) {
    ...
    for (let index = 0; index < arrFullName.length; index++) {
      if (arr[index] != "") {
        arrFullNameRectified.push(arr[index]);
      }
    }
